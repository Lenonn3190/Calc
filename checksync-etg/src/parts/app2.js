...
function renderChecklist(root){
  ...
  box.querySelectorAll('button[data-addph]').forEach(btn=>{ btn.onclick=()=>{
    ...
    // sem 'capture': o navegador oferece Câmera OU Galeria; multiple permite várias de uma vez
    const inp=document.createElement('input'); inp.type='file'; inp.accept='image/*'; inp.multiple=true;
    inp.onchange=async()=>{ const files=[...inp.files]; if(!files.length) return; ins.itens[idx].fotos ||= [];
      for(const f of files){ if(!/^image\//.test(f.type||'')) continue; const d=await shrink(await readData(f),1000,0.72); ins.itens[idx].fotos.push(d); }
      renderChecklist(root); updateProgress(root); };
    inp.click();
  }; });
